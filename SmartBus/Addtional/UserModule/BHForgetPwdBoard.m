...
@implementation BHForgetPwdBoard
...
ON_SIGNAL2( BeeUIBoard, signal )
{
	...
	{
        [self indicateIsFirstBoard:NO image:[UIImage imageNamed:@"nav_profile.png"] title:@"找回密码"];
...
- (void)handleRequest:(BeeHTTPRequest *)request
{
	if ( request.succeed )
	{
        if ([request.userInfo isEqualToString:@"checkPhone"])
        {
            ...
            {
                [self presentMessageTips:@"您输入的手机号不存在"];
                return;
            }
