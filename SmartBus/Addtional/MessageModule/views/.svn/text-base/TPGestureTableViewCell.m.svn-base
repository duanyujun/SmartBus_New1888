...
- (void)deleteAction
{
    if ([delegate respondsToSelector:@selector(deleteMsgWithIndex:)]) {
        [delegate deleteMsgWithIndex:index];
    }
}
...
-(void)layoutBottomView{
    if(!self.bottomRightView){
        ...
        UIButton *button = [[UIButton alloc]initWithFrame:CGRectMake(260.f, 11.5f, 40.f, 30.f)];
        [button setTitle:@"删除" forState:UIControlStateNormal];
        [button addTarget:self action:@selector(deleteAction) forControlEvents:UIControlEventTouchUpInside];
        [_bottomRightView addSubview:button];
        [button release];
    }
...
- (void)panGestureHandle:(UIPanGestureRecognizer *)recognizer
{
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan) {
        ...
        if (_lastDirection == LMFeedCellDirectionLeft) {
