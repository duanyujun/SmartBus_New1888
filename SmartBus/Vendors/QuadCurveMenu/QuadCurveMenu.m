...
#define NEARRADIUS 80.0f
#define ENDRADIUS 80.0f
#define FARRADIUS 60.0f
...
- (id)initWithFrame:(CGRect)frame start:(CGPoint)point menus:(NSArray *)aMenusArray
{
    ...
    if (self) {
        ...
        for (int i = 0; i < count; i ++)
        {
            ...
            item.endPoint = CGPointMake(point.x + ENDRADIUS * sinf(i * M_PI_2 / (count - 1)), point.y - ENDRADIUS * cosf(i * M_PI_2 / (count - 1)));
            item.nearPoint = CGPointMake(point.x + NEARRADIUS * sinf(i * M_PI_2 / (count - 1)), point.y - NEARRADIUS * cosf(i * M_PI_2 / (count - 1)));
            item.farPoint = CGPointMake(point.x + FARRADIUS * sinf(i * M_PI_2 / (count - 1)), point.y - FARRADIUS * cosf(i * M_PI_2 / (count - 1)));
        ...
        // add the "Add" Button.
        _addButton = [[QuadCurveMenuItem alloc] initWithImage:[UIImage imageNamed:@"bg-menuitem.png"]
...
{
    return [self initWithFrame:frame start:STARTPOINT menus:aMenusArray];
}
