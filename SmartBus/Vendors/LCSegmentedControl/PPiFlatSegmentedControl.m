...
- (id)initWithFrame:(CGRect)frame items:(NSArray*)items iconPosition:(IconPosition)position {
    return [self initWithFrame:frame items:items iconPosition:position andSelectionBlock:nil];
}

- (void)segmentDidSelectBlock:(selectionBlock)block {
    _selBlock=block;
}
