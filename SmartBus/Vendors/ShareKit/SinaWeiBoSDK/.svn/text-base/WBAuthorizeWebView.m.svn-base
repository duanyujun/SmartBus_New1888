...
- (id)init
{
    if (self = [super initWithFrame:CGRectMake(0, 0, 320, 480)])
    {
        ...
        [button addTarget:self action:@selector(onCloseButtonTouched:) forControlEvents:UIControlEventTouchUpInside];
...
{
    [self hide:YES];
    if ([delegate respondsToSelector:@selector(authorizeWebView:didFailWithError:)])
    {
        [delegate authorizeWebView:self didFailWithError:nil];
    }
}
