...
//    BOOL refreshTokenSaveOK = [SFHFKeychainUtils storeUsername:TCKeychainRefreshToken andPassword:refreshToken forServiceName:serviceName updateExisting:YES error:nil];
    ...
    [SFHFKeychainUtils storeUsername:@"isSSOAuth" andPassword:[NSString stringWithFormat:@"%c", isSSOAuth] forServiceName:serviceName updateExisting:YES error:nil];
    ...
    return accessTokenSaveOK && openIdSaveOK && nameSaveOK;
}
...
- (void)readAuthorizeDataFromKeychain{
    ...
    NSString *SSOAuth =  [SFHFKeychainUtils getPasswordForUsername:@"isSSOAuth" andServiceName:serviceName error:nil];

    self.isSSOAuth = [SSOAuth isEqualToString:@"YES"]?YES:NO;
...
- (BOOL)deleteAuthorizeDataInKeychain{
    ...
    [SFHFKeychainUtils deleteItemForUsername:@"isSSOAuth" andServiceName:serviceName error:nil];
...
- (void)initRequestWithMethodName:(NSString *)methodName
    ...
    if ((refreshToken != nil) || (expireTime == 0) || isSSOAuth == YES ) {
//      如果从未登录过或登出过
        if (G_LOGOUT) {
            [self readAuthorizeDataFromKeychain];
        }
        ...
        if ([self isAuthorizeExpired]) {  
            //      授权过期去刷授权 
            code = [self refreshAccessToken:self.appKey
...
- (void)postTextTweetWithFormat:(NSString *)format 
    ...
    [self initRequestWithMethodName:@"t/add"
                         httpMethod:@"POST" 
                             params:dic
                       postDataType:TCkWBRequestPostDataTypeNormal
...
//    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    [self initRequestWithMethodName:@"t/add_pic"
                         httpMethod:@"POST" 
                             params:dic
                       postDataType:TCkWBRequestPostDataTypeMultipart
                andHttpHeaderFields:nil
                           delegate:requestDelegate
                          onSuccess:successCallback
                          onFailure:failuerCallback];
...
- (void)getHomeTimelinewithFormat:(NSString *)format
    ...
    [self initRequestWithMethodName:@"statuses/home_timeline"
                         httpMethod:@"GET"
                             params:dic
                       postDataType:TCkWBRequestPostDataTypeNone
                andHttpHeaderFields:nil
                           delegate:requestDelegate
                          onSuccess:successCallback
...
- (void)logInWithDelegate:(id) requestDelegate  
    ...
    {
        bRet = [WBApi loginApp:appKey secret:appSecret reserver:nil];
    }
...
//    [nav release];
    ...
    TCWBAuthorizeView *authorizeView = [[TCWBAuthorizeView alloc] init];
    [authorizeView setDelegate:self];
...
//        
//       BOOL bRet = NO;
//       NSLog(@"wbapi is %@",[WBApi isWeiboAppSupport]?@"YES":@"NO");
//       
//        if( (bRet = [WBApi isWeiboAppSupport]))
//        {
//            bRet = [WBApi loginApp:appKey secret:appSecret reserver:nil];
...
//           // [ setDelegate:authDelegate];
//            //[rootViewController presentModalViewController:nav animated:YES];
//            //[authViewController release];
//           // [nav release];
//            CFRunLoopRun();
//            if (authDelegate.token != nil) {
//                [self saveWBTokenInfo:authDelegate.token name:authDelegate.name];
//                BOOL saveKeychainOK = [self saveAuthorizeDataToKeychain];
//                    if (!saveKeychainOK) {
//                        BOOL deleteKeychainOK = [self deleteAuthorizeDataInKeychain];
//                        if (deleteKeychainOK) {
...
//
//    
//  //NSLog(@"islogin is %@",[self isLoggedIn]?@"YES":@"NO");
//
//    if (![self isLoggedIn])
//    {
//        // if(isUIDelegate == YES){
