...
- (void)imageDownloader:(SDWebImageDownloader *)downloader didDownLoading:(NSNumber*)percent
{
    ...
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        if (aDownloader == downloader)
        {
            ...
            if ([delegate respondsToSelector:@selector(imageDownloader:didDownLoading:)]) {
                [delegate performSelector:@selector(imageDownloader:didDownLoading:) withObject:self withObject:percent];
            }
        }
    }
}
